---
name: security-audit
description: Audit a WordPress theme or plugin PHP for common vulnerabilities. Use this when the user asks to review a plugin or theme for security, check for XSS, CSRF, SQL injection, or unsafe input handling, or prepare code for submission to wordpress.org. Flags unescaped output, unsafe superglobal handling, missing nonces, unprepared SQL, capability gaps, and missing ABSPATH guards. Produces a grouped pass, warn, and fail report.
...
### Rule 6. Validate and sanitize input before use (fail when request input reaches a sink unchanged)

Grep for `$_GET`, `$_POST`, `$_REQUEST`, `$_COOKIE`, `$_FILES`, and `$_SERVER`. A superglobal is not automatically a vulnerability: it must be unslashed and validated or sanitized for its intended use before it reaches output, a query, a file operation, an option update, or a redirect.
...
Fail when request input reaches a sink unchanged. Warn when the context needs a stricter validator (for example, a URL accepted with `sanitize_text_field`) or when a read-only endpoint handles user input without an obvious purpose. A raw superglobal that is only checked with `isset()` before being sanitized is a pass.
...
- Do not treat every raw superglobal match as a vulnerability. Confirm the data flow and the intended context before reporting it.
