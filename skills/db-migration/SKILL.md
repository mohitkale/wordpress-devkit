...
3. Output the four-step plan: backup, dry-run, real run, flush caches. Keep it in the conversation for the user to review before running.
...
Present a single copyable `migration-plan.sh` example in the conversation, with the three commands and the post-migration tasks commented by step. Do not create files. Example content:
...
2. Presents a `migration-plan.sh` example with the four steps above.
