...
- A GitHub marketplace catalog so Claude Code users can install the plugin from this repository and receive versioned updates.
- A Docker Desktop smoke suite that boots WordPress and MariaDB, activates a fixture plugin and block theme, renders a dynamic block, exercises cache commands, and proves a URL migration dry-run leaves data unchanged.
- Packaging script for a direct ZIP session load with `claude --plugin-dir`.

### Changed

- Rewrote the README as a user-facing installation and workflow guide, including the tested Docker flow.
- Removed the optional public email from plugin metadata and documentation; GitHub Issues is the support route.
- Tightened the migration skill so it presents a plan without creating files, expanded the security audit to cover unsafe request input, and removed stale upgrade-version assumptions.
