...
process.stdin.on("end", () => {
  try {
    ...
    if (/\bwp\s+search-replace\b/.test(cmd) && !/--dry-run/.test(cmd)) {
      notes.push("Real `wp search-replace` run (no --dry-run flag). If this touched the database, confirm that a backup exists. For URL migrations, `/wordpress-devkit:db-migration <old> <new>` always generates the `--dry-run` step first.");
    }
    if (/\bwp\s+search-replace\b/.test(cmd) && /--dry-run/.test(cmd)) {
